  ...
  <section class="knowledge-document-pane" data-testid="document-grid" @dragenter.prevent="$emit('drag-active', true)" @dragover.prevent @dragleave.prevent="$emit('drag-active', false)" @drop.prevent="$emit('drop', $event)">
    ...
    <div v-else class="document-grid">
      <article v-for="item in documents" :key="item.id" class="document-card" :class="{ selected: selectedId === item.id }">
        ...
        <div v-if="jobFor(item.id)" class="import-progress"><div role="progressbar" :aria-label="`${item.display_name} 导入进度`" aria-valuemin="0" aria-valuemax="100" :aria-valuenow="jobFor(item.id)!.progress"><i :style="{ width: `${jobFor(item.id)!.progress}%` }" /></div><span>{{ jobFor(item.id)!.progress }}%</span><button type="button" :aria-label="`取消 ${item.display_name} 的导入`" @click="cancelJob(jobFor(item.id)!.id)">取消</button></div>
      </article>
    </div>
  </section>
</template>
...
const props=defineProps<{documents:KnowledgeDocument[];jobs:KnowledgeImportJob[];selectedId:number|null;cancelJob:(id:number)=>unknown}>()
defineEmits<{(event:'select',id:number):void;(event:'drop',value:DragEvent):void;(event:'drag-active',value:boolean):void}>()
function jobFor(documentId:number){return props.jobs.find(job=>job.document_id===documentId&&['QUEUED','VALIDATING','PARSING','OCR_RUNNING','INDEXING'].includes(job.status))}
