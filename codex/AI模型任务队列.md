...
| S-018 | P0 | 进行中 | 设计本地知识库、文件导入、解析、索引与学习流程接入 | 现有 FastAPI/SQLite/Electron、安全边界、用户新增需求 | 支持格式与限额、文件存储和解析隔离、文本切片/检索、引用溯源、会话/资源接入、删除与重建、接口/UI、测试和打包方案 |
...
| T-035 | P0 | 阻塞 | 实现本地知识库与安全文件导入 | S-018 | 文件选择/拖放、格式与大小校验、解析任务、SQLite 元数据和本地索引、检索引用、学习助手接入、删除/重建、自动化与桌面打包验收 |
