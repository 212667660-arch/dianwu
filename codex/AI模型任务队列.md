...
> 当前模型批次：Sol（S-031 桌面稳定与本地教材导入进行中）
> 最后审视日期：2026-07-17
...
| S-031 | P0 | 进行中 | 修复墨团连续点击视觉放大并完成本地教材 500 MiB 导入、预览和外部阅读器闭环 | T-035、T-036、用户真实教材目录与本轮诊断 | 固定画布/窗口不变量、交互幂等测试、单文件与批次 500 MiB、真实 77/188 MB PDF 导入、应用内预览/只读外部打开及桌面打包验收 |
| S-032 | P0 | 就绪 | 接入合法初高中数学教材目录并增强知识点总结和分步解题 | S-031、现有知识检索与内容安全边界 | 人教社官方目录、开放许可下载白名单、教材集合绑定、知识点总结、例题/解题步骤、可信引用和端到端验证 |
