...
Electron 启动时会同时创建独立的透明、无边框、置顶桌宠窗口。默认原创角色“墨团”支持空闲、左右移动、挥手、跳跃、失败、等待、执行和检查动画；学习生成、协议校验、完成与失败会自动切换任务状态。单击挥手、双击跳跃，按住拖动可移动到其他显示器；位置、显示开关、缩放、动画速度以及两路声音设置会保存在 `userData/pet-settings.json`。隐藏时暂停动画和声音，空闲 60 秒后降低刷新频率。

书桌右栏的“桌面学习伙伴”卡片可显示/隐藏桌宠，选择 50%–150% 缩放和 0.5×–2× 动画速度，也可分别调节动作音效与语音鼓励。动作音效默认开启、音量 50%；语音默认关闭、预设音量 75%，升级后不会突然说话。语音使用操作系统本地 `speechSynthesis`，不会上传文字或音频；系统没有可用中文语音时静默降级。等待和失败等低频状态会触发鼓励，普通语音有 30 秒冷却，失败提示可及时打断普通鼓励。

替换角色时，点击卡片中的“导入角色包”，选择只包含以下两个文件的文件夹：
...
- 桌宠后续可增加多角色长期角色库、自定义台词和系统音色选择；主观音色、逐帧视觉与全面许可证 QA 仍待专门核对。账号体系继续只保留扩展边界。
- 根据真实模型输出扩充评测集和评分基线。
