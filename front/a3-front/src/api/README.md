...
- 桌宠设置、角色导入和任务状态只通过 `petGet`、`petUpdateSettings`、`petChooseCharacter`、`petResetCharacter`、`petSetTaskState` 五个固定 bridge；角色目录由 Electron 主进程选择、校验并原子替换，renderer 不提交或接收文件路径。Web 模式返回不可用快照并安全忽略状态上报，不打开目录选择器，也不经通用 HTTP transport。
- 桌宠动作音效和系统本地语音分别使用 `soundEnabled/soundVolume` 与 `voiceEnabled/voiceVolume`，两路音量独立于动画速度。语音默认关闭，不联网；桌宠隐藏或通道关闭时停止对应声音。桌宠自身使用独立 `a3Pet` preload，仅暴露 ready、拖动和状态/设置订阅，不获得主应用 API、模型配置或文件能力。
